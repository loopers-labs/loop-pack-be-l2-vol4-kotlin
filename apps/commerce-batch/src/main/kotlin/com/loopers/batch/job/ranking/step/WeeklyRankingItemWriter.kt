...
class WeeklyRankingItemWriter(
    private val jdbcTemplate: JdbcTemplate,
    @Qualifier(RedisConfig.REDIS_TEMPLATE_MASTER)
    private val redisTemplate: RedisTemplate<String, String>,
    @Value("#{jobParameters['baseDate']}") baseDate: String,
) : ItemWriter<ProductMetricsWeekly>, StepExecutionListener {
    private val zsetKey = ZSET_KEY_PREFIX + AggregationPeriod.weeklyOf(LocalDate.parse(baseDate)).key
    ...
    companion object {
        private const val ZSET_KEY_PREFIX = "ranking:weekly:"
        private val UPSERT_SQL = """
            INSERT INTO product_metrics_weekly
                (product_id, year_week, like_count, sales_count, view_count, score, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
