...
- **코드 채택 (확정 2026-07-03)**: 변형 B를 프로덕션 경로로 채택, `feature/week07-event-driven`에 merge. 근거 — 동일 정합성 + 500 폭풍 94% 감소 + 꼬리 지연 개선, 그리고 Phase 3(Redis) 설계가 "DB 조건부 UPDATE + unique 최종 방어"를 방어선으로 전제.
- **다음**: 변형 C는 스킵(사용자 결정, WRITING-LOG 결정 14). Phase 2 진단 사다리·절대치 재측정은 EC2 재생성이 필요해 보류 — 과제 MUST인 Phase 4(Kafka 지연 발급)로 진행.
...
- 측정 절차는 사전 작성한 런북으로 표준화해 실행. 스냅샷 타이밍(원격 sleep 누적 오차)이 계획과 어긋나 스냅샷2가 무효 — 다음 런은 k6 시작 시각 기준 절대 시각 스케줄로 개선.
