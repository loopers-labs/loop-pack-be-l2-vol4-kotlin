...
- 상품별 좋아요 수를 매 조회마다 집계하지 않고, 좋아요 레코드에서 파생되는 eventually consistent projection 테이블로 비정규화할 수 있는가?
...
- 상품별 좋아요 수는 `product_like_counts` projection 테이블에 비정규화해 보관한다. `likes` 레코드가 사용자-상품 좋아요 상태의 단일 진실 원천이며, 실제 INSERT/DELETE 영향 행 수가 `1`인 상태 전이만 `LIKE_COUNT_CHANGED_V1` 이벤트를 남긴다. Kafka consumer가 `commerce.like-count-events.v1`에서 `productId` key 이벤트를 소비해 `delta=+1/-1`을 반영하므로 `product_like_counts`는 eventually consistent projection이다.
- 좋아요 수 이벤트는 UUID `eventId`를 갖는다. consumer는 `processed_kafka_events`에 `(consumer_group, event_id)`를 저장해 Kafka 재전달 시 같은 `delta`를 중복 반영하지 않는다.
...
- `LIKE_COUNT_CHANGED_V1`은 `likes` 상태 변경과 같은 DB 트랜잭션에서 `outbox_events`에 먼저 저장하고, 별도 relay가 Kafka broker ack 이후 발행 완료로 표시한다. 이벤트 유실을 막기 위한 저장소 권위는 outbox row이며, Kafka publish는 좋아요 쓰기 트랜잭션 밖에서 수행한다.
- 쿠폰 발급·사용의 권위 상태는 좋아요 수 projection 이벤트와 분리한다. 발급 쿠폰 상태 변경은 주문/쿠폰 유스케이스의 동기 트랜잭션 경계를 유지한다.
...
erDiagram
    ...
    PRODUCTS ||--|| PRODUCT_LIKE_COUNTS : has_count
...
- `product_like_counts`는 `product_id`를 PK로 갖는 상품 1:1 projection 테이블이다. 좋아요 수를 매 조회마다 `COUNT`로 계산하는 대신 비정규화 컬럼으로 보관해 목록·`likes_desc` 정렬의 읽기 비용을 낮춘다. `likes` 레코드가 진실 원천이고, `product_like_counts`는 Kafka consumer가 `LIKE_COUNT_CHANGED_V1`을 처리한 뒤 갱신하는 eventually consistent projection이다. 좋아요 등록은 `delta=+1`, 취소는 `delta=-1`이며, INSERT/DELETE 영향 행 수가 0인 멱등 no-op 요청은 이벤트를 만들지 않는다. 상품 생성 시 `like_count = 0` 행을 함께 만들고, consumer 장애나 누락 의심 시 Kafka replay 또는 `likes` 기준 backfill/rebuild로 projection을 복구한다.
- `outbox_events`는 좋아요 쓰기 트랜잭션에서 생성되는 durable event 저장소다. `LIKE_COUNT_CHANGED_V1` row는 UUID `eventId`, `event_type`, `aggregate_type=PRODUCT`, `aggregate_id=productId`, `topic=commerce.like-count-events.v1`, `kafka_key=productId`, `payload(delta, userId, occurredAt)`를 보관한다. relay는 Kafka broker ack 이후에만 `PUBLISHED`로 변경하고, 실패 시 retry metadata를 갱신한다.
- `processed_kafka_events`는 Kafka consumer의 idempotency 테이블이다. `LIKE_COUNT_CHANGED_V1`의 UUID `eventId`를 `event_id`에 저장하고, 같은 `consumer_group`에서 이미 처리한 이벤트면 `product_like_counts`에 `delta`를 다시 적용하지 않는다. 운영 DDL에서는 `(consumer_group, event_id)` unique 제약을 둔다.
...
| `product_like_counts(product_id)` | primary key | 상품 1:1 좋아요 수 projection 조회·갱신 기준 |
...
| `processed_kafka_events(consumer_group, event_id)` | unique | LIKE_COUNT_CHANGED_V1 재전달 시 delta 중복 반영 방지 |
...
| Outbox 이벤트 | `OutboxEventJpaEntity` 값 보관 | 좋아요 상태 전이와 같은 트랜잭션에서 `LIKE_COUNT_CHANGED_V1` 저장. Kafka publish는 relay가 트랜잭션 밖에서 수행 |
| 좋아요 projection - 상품 | `ProductLikeCountJpaEntity` shared PK `productId` 값 보관 | 상품과 1:1 projection 행. Kafka consumer가 `LIKE_COUNT_CHANGED_V1`의 `delta`를 반영하고, 조회는 상품 목록/상세 조립 시 bulk 조회 |
| Kafka 처리 이력 - 이벤트 | `ProcessedKafkaEventJpaEntity` 값 보관 | UUID `eventId`, `consumerGroup`, `eventType`, `productId`, `processedAt` 저장. `(consumerGroup, eventId)` unique로 중복 처리 방지 |
...
- 좋아요 수는 `product_like_counts` projection 테이블로 비정규화해 매 요청 `COUNT` 계산을 제거한다. 대신 쓰기 직후 상품 목록·상세 응답의 좋아요 수가 `likes` 최신 상태보다 늦을 수 있다. consumer lag, Kafka 재전달, backfill/rebuild 절차를 운영 지표와 runbook으로 관리해야 하며, 초인기 상품에서 projection 갱신 병목이 관측되면 분산 카운터(샤딩)나 Redis 카운터를 추가로 검토한다.
...
- `likes_desc` 정렬은 `product_like_counts.like_count`를 기준으로 수행해 실시간 집계 조인을 피한다. 정렬 성능이 더 필요해지면 `product_like_counts(like_count)` 보조 인덱스나 커버링 인덱스를 검토한다.
