...
@TestPropertySource(
    properties = [
        "spring.batch.job.name=${ProductRankGroupByOneShotJobConfig.JOB_NAME}",
        // 부팅 시 자동 실행 방지 — 파라미터 없는 startup 런이 validator에 걸려 컨텍스트가 죽는다
        "spring.batch.job.enabled=false",
    ],
)
class ProductRankGroupByOneShotJobE2ETest @Autowired constructor(
    private val jobLauncherTestUtils: JobLauncherTestUtils,
    @param:Qualifier(ProductRankGroupByOneShotJobConfig.JOB_NAME) private val job: Job,
    ...
    @DisplayName("주간 기간의 메트릭만 단발 GROUP BY로 가중치 집계되어, 다른 잡들과 동일한 TOP 랭킹이 MV에 저장된다.")
    @Test
    fun aggregatesWeeklyWindowOnly_andConfirmsRanking() {
        jobLauncherTestUtils.job = job
        // 집계 창: 2026-07-13(월) ~ 07-19(일). 기본 가중치 VIEW=10, LIKE=50, SALES=500.
        // ProductRankGroupByJobE2ETest와 동일 시나리오 — 변형 간 결과 동등성을 보장한다.
