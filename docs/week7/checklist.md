...
- [x] **17. Phase 1 — DB-only 부하 비교 (종료)** — A 비관(FOR UPDATE) **✅ (EXP-01, 정합성 완벽·병목=단일행 락 직렬화, CPU 아님)** / B 조건부 원자 UPDATE **✅ (EXP-02, S2 무릎 실측 ~550~590/s·500 폭풍 94%↓·정합성 완벽, 병목 락 대기→CPU로 이동, 스파이크 한계는 여전)** / C 낙관(@Version) **스킵(사용자 결정, 시간 제약 — WRITING-LOG 결정 14)**. DB-only 구조적 상한 = B 기준 ~550~590/s(co-located 하한 추정). EC2 terminate + SG 삭제 완료
