...
sequenceDiagram
    ...
    alt 주문 목록 조회
        ...
        OrderController->>OrderFacade: getOrders(memberId, period)
        OrderFacade->>OrderService: getOrders(memberId, period)
        OrderService->>OrderRepo: findByMemberId(memberId, period)
        OrderRepo-->>OrderService: order summaries
        OrderService-->>OrderFacade: list result
        OrderFacade-->>OrderController: list result
        OrderController-->>User: 목록 응답
    else 주문 상세 조회
        User->>OrderController: GET /api/v1/orders/{orderId}
        OrderController->>OrderFacade: getOrderDetail(memberId, orderId)
        OrderFacade->>OrderService: getOrderDetail(memberId, orderId)
        OrderService->>OrderRepo: findDetailByMemberIdAndOrderId(memberId, orderId)
        OrderRepo-->>OrderService: order detail with snapshots
        OrderService-->>OrderFacade: detail result
        OrderFacade-->>OrderController: detail result
