...
class ProductMetricDailyOrderEventConsumerTest {
    private val service = mock<ProductMetricDailyEventService>()
    private val acknowledgment = mock<Acknowledgment>()
    private val consumer = ProductMetricDailyOrderEventConsumer(service)

    @DisplayName("Order metric batch 처리에 성공하면 ack 한다")
    @Test
    ...
    @DisplayName("Order metric batch 중 non-retryable 실패가 발생하면 실패 index를 전파하고 ack 하지 않는다")
    @Test
