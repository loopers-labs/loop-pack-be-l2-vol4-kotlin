...
class ProductRankingDailyIntegrationTest @Autowired constructor(
    ...
    private fun accumulate(eventId: String, vararg changes: ScoreChange, occurredAt: Instant = NOON_KST) =
        rankingAccumulateService.accumulate(eventId, occurredAt, changes.toList())
    ...
    @DisplayName("조회 변화량(+0.1)을 적재하면, 오늘 판에 +0.1 과 내일 판에 +0.01 을 함께 적재한다 — dual write.")
    @Test
    fun accumulatesViewScoreToTodayAndCarryToTomorrow() {
        accumulate("e-1", ScoreChange(1, RankingWeights.VIEW))
    ...
    @DisplayName("좋아요 변화량(+0.2)을 적재하면, 오늘 판에 +0.2 와 내일 판에 +0.02 를 적재한다.")
    @Test
    fun accumulatesLikeScore() {
        accumulate("e-1", ScoreChange(1, RankingWeights.LIKE))
    ...
    @DisplayName("좋아요 취소 변화량(-0.2)이 단독으로 오면, 오늘 판 -0.2 와 내일 판 -0.02 로 음수 score 행을 허용한다.")
    @Test
    fun accumulatesNegativeScore_whenUnlikedAlone() {
        accumulate("e-1", ScoreChange(1, RankingWeights.LIKE.negate()))
    ...
    @DisplayName("한 이벤트의 변화량 목록은 각 상품에 개별 적재된다 — 주문 order line 당 +0.7.")
    @Test
    fun accumulatesEachChangeOfSingleEvent() {
        accumulate("e-1", ScoreChange(1, RankingWeights.ORDER_LINE), ScoreChange(2, RankingWeights.ORDER_LINE))
    ...
    fun accumulatesScoreOnSameRow_whenEventsRepeat() {
        accumulate("e-1", ScoreChange(1, RankingWeights.VIEW))
        accumulate("e-2", ScoreChange(1, RankingWeights.VIEW))
    ...
    fun singleOrderOutweighsThreeLikes() {
        accumulate("e-1", ScoreChange(1, RankingWeights.ORDER_LINE))
        accumulate("e-2", ScoreChange(2, RankingWeights.LIKE))
        accumulate("e-3", ScoreChange(2, RankingWeights.LIKE))
        accumulate("e-4", ScoreChange(2, RankingWeights.LIKE))
    ...
    fun splitsRankingDateByKst() {
        accumulate("e-1", ScoreChange(1, RankingWeights.VIEW), occurredAt = Instant.parse("2026-07-15T14:59:00Z"))
        accumulate("e-2", ScoreChange(2, RankingWeights.VIEW), occurredAt = Instant.parse("2026-07-15T15:00:00Z"))
