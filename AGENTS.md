...
- 애플리케이션 서비스는 서로 다른 도메인을 조립하고, 도메인 로직을 조정하여 기능을 제공해야 한다.
- 동일하거나 유사한 규칙이 여러 서비스에 반복해서 나타나면, 해당 규칙은 도메인 객체의 책임일 가능성이 높다고 판단한다.
- 각 기능의 책임 배치와 결합도에 대해서는 개발자의 의도를 확인한 뒤 구현을 진행한다.

## 아키텍처, 패키지 구성 전략

- 본 프로젝트는 레이어드 아키텍처를 따르며, DIP(의존성 역전 원칙)를 준수한다.
- API request/response DTO와 application 레이어 DTO는 분리해서 작성한다.
- 패키징은 4개 레이어 패키지를 기준으로 두고, 각 레이어 하위에 도메인별 패키지를 구성한다.
- 기본 패키지 구조는 다음 예시를 따른다.

```text
/interfaces/api           (presentation 레이어 - API)
/application/<domain>     (application 레이어 - 도메인 레이어를 조합해 사용 가능한 기능 제공)
/domain/<domain>          (domain 레이어 - 도메인 객체, 엔티티, Repository 인터페이스 위치)
/infrastructure/<domain>  (infrastructure 레이어 - JPA, Redis 등을 활용한 Repository 구현체 제공)
```
