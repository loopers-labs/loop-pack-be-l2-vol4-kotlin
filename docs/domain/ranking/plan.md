...
- [ ] (streamer) 상품을 랭킹판에서 제거하면(오늘·어제 키) 이후 Top-N·순위 조회에 나오지 않는다
- [ ] (commerce-api) Top-N 조회는 점수 내림차순으로 상품 식별자·점수를 반환한다
- [ ] (commerce-api) 페이지 구간(시작 위치·크기)에 해당하는 항목만 반환한다
- [ ] (commerce-api) 가장 점수 높은 상품의 순위는 1 이다 — 순위는 1-based 로 반환된다
- [ ] (commerce-api) 랭킹판에 없는 상품의 순위 조회는 null 을 반환한다
- [ ] (commerce-api) 랭킹판 전체 항목 수를 조회할 수 있다
- [ ] (commerce-api) 존재하지 않는 날짜의 랭킹판 조회는 빈 목록을 반환한다
- [ ] (streamer) 전일 랭킹판 점수에 이월 가중치를 곱해 다음 날 랭킹판으로 복사한다
- [ ] (streamer) 이월 후 유입되는 점수는 이월 점수 위에 누적된다
- [ ] (streamer) 전일 랭킹판이 없으면 이월은 아무것도 만들지 않는다
- [ ] (streamer) 다음 날 랭킹판이 이미 존재하면 이월은 아무것도 하지 않는다 — 중복 실행·자정 후 오발동이 실점수를 덮어쓰지 않는다

### Phase 4 — Application Facade (`com.loopers.application.ranking`)
> 유스케이스 진입점. 신호 반영 조율(쓰기) / 랭킹 조립(읽기). 트랜잭션 경계·예외 전파.

- [ ] (streamer) 조회 신호가 들어오면 발생 시각 날짜의 랭킹판에 조회 가중치만큼 누적된다
- [ ] (streamer) 좋아요 취소 신호는 해당 날짜 랭킹판 점수를 좋아요 가중치만큼 차감한다
- [ ] (streamer) 주문 신호는 주문에 담긴 상품별로 수량 × 주문 가중치만큼 각각 누적한다
- [ ] (streamer) 같은 eventId 가 재전달되면 랭킹 점수가 다시 반영되지 않는다 — 자체 멱등 표식(Redis)이 걸러낸다
- [ ] (streamer) 상품 삭제 신호가 들어오면 오늘·어제 랭킹판에서 그 상품을 제거한다
- [ ] (commerce-api) 상품을 삭제하면 삭제 사실이 외부 이벤트(`PRODUCT_DELETED`)로 발행된다 — outbox 적재
