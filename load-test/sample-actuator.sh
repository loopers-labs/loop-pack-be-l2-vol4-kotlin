...
OUT="${1:?출력 파일 경로 필요}"
DUR="${2:-300}"
ACT="http://localhost:8081/actuator/prometheus"
echo "epoch,orders_count,orders_sum,orders_max,products_count,hikari_pending,hikari_active,tomcat_busy" > "$OUT"
END=$(( $(date +%s) + DUR ))
while [ "$(date +%s)" -lt "$END" ]; do
  ...
  echo "$TS,${OC:-0},${OS:-0},${OM:-0},${PC:-0},${HP:-0},${HA:-0},${TB:-0}" >> "$OUT"
  sleep 5
done
