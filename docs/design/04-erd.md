...
- 상품별 좋아요 수를 매 조회마다 집계하지 않고, 좋아요 레코드와 정합을 유지하는 집계 테이블로 비정규화할 수 있는가?
...
- 상품별 좋아요 수는 `product_like_counts` 집계 테이블에 비정규화해 보관한다. `likes` 레코드가 단일 진실 원천이며, 좋아요 등록/취소와 같은 트랜잭션에서 집계 행을 원자적으로 갱신한다. 갱신량은 `exists` 선조회가 아니라 `likes`에 대한 INSERT/DELETE의 영향 행 수(0 또는 1)로 결정해 동시·반복 요청에도 카운트가 어긋나지 않게 한다.
...
- `product_like_counts`는 `product_id`를 PK로 갖는 상품 1:1 집계 테이블이다. 좋아요 수를 매 조회마다 `COUNT`로 계산하는 대신 비정규화 컬럼으로 보관해 목록·`likes_desc` 정렬의 읽기 비용을 낮춘다. `likes` 레코드가 진실 원천이고, 좋아요 등록/취소와 같은 트랜잭션에서 `like_count`를 `+1`/`-1` 원자 갱신한다. 증감은 `likes` INSERT/DELETE의 영향 행 수(0 또는 1)로 결정하며, 취소는 `like_count > 0` 조건으로 음수를 방지한다. 상품 생성 시 `like_count = 0` 행을 함께 만들어 갱신 대상 행 부재를 막는다.
...
| `product_like_counts(product_id)` | primary key | 상품 1:1 좋아요 수 집계 행 단건 갱신 기준 |
...
| 좋아요 집계 - 상품 | `ProductLikeCountJpaEntity` shared PK `productId` 값 보관 | 상품과 1:1 집계 행. 좋아요 등록/취소 트랜잭션에서 `like_count` 원자 갱신, 조회는 상품 목록/상세 조립 시 bulk 조회 |
...
- 좋아요 수는 `product_like_counts` 집계 테이블로 비정규화해 매 요청 `COUNT` 계산을 제거한다. 대신 인기 상품은 좋아요 등록/취소가 동일 집계 행에 직렬화되므로 hot row 경합이 생길 수 있다. 평상시 트래픽에서는 행 단위 원자 갱신으로 충분하며, 초인기 상품에서 병목이 관측되면 분산 카운터(샤딩)나 Redis 카운터, 비동기 집계를 추가로 검토한다.
