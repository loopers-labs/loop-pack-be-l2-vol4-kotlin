...
class ProductRankingKafkaConsumerTest {
    ...
    @DisplayName("좋아요는 +0.2, 취소는 -0.2 의 점수 델타로 변환해 랭킹 적재를 위임한다.")
    @Test
    fun convertsLikeEventsToScoreDeltas() {
        ...
        assertAll(
            { verify(rankingAccumulateService).accumulate(eq("e-1"), any(), eq(listOf(ScoreDelta(1, BigDecimal("0.2"))))) },
            { verify(rankingAccumulateService).accumulate(eq("e-2"), any(), eq(listOf(ScoreDelta(1, BigDecimal("-0.2"))))) },
            { verify(acknowledgment).acknowledge() },
        )
    }

    @DisplayName("주문은 수량·단가와 무관하게 order line 당 +0.7 델타 목록으로 변환한다.")
    @Test
    fun convertsOrderLinesToFixedScoreDeltas() {
        ...
        verify(rankingAccumulateService).accumulate(
            eq("e-1"),
            any(),
            eq(listOf(ScoreDelta(1, BigDecimal("0.7")), ScoreDelta(2, BigDecimal("0.7")))),
        )
    }

    @DisplayName("조회는 +0.1 델타로 변환하고, record timestamp 를 발생 시각으로 전달한다.")
    @Test
    fun convertsViewToScoreDeltaWithRecordTimestamp() {
        ...
        verify(rankingAccumulateService).accumulate(
            ...
            eq(listOf(ScoreDelta(1, BigDecimal("0.1")))),
        )
    }
