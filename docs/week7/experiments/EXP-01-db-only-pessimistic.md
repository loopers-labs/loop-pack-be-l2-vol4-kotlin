...
- **[정정, EXP-02에서]** §4의 "처리량 천장 ≈ 220~270 req/s" 중 S2 쪽 ~270/s는 램프 스케줄의 평균 도착률과 일치하는 값이라 천장의 증거가 못 된다(A도 의도 발사량 69k 중 65.8k를 완료 — 스케줄을 거의 소화). 천장 문구는 "S1 스파이크 하 실효 처리량"으로 한정해 읽을 것. A의 S2 무릎은 타임라인 미보존으로 실측 불가 — EXP-02부터 k6 진행 로그 차분으로 무릎을 실측.
